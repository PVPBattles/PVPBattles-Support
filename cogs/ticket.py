...
def auto_answers(config):
    value = config.get('ticket_auto_answers', [])
    ...
    for item in value:
        ...
        if keyword and response:
            result.append({'keyword': keyword[:100], 'response': response[:2000]})
    return result[:25]
...
class TicketCloseView(discord.ui.View):
    ...
    async def close(self, interaction):
        ...
        is_staff_member = staff(interaction.user, c.get('ticket_staff_role_ids', []))
        if not is_staff_member:
            if creator == interaction.user.id:
                await interaction.response.send_message('❌ チケットを作成した本人でも、スタッフロールがない場合は閉じられません。', ephemeral=True); return
...
class AutoAnswerAddModal(discord.ui.Modal):
    def __init__(self, current=None):
        super().__init__(title='自動回答を追加')
        self.keyword = discord.ui.TextInput(label='キーワード', placeholder='例：IP / 料金 / 参加方法', max_length=100, required=True)
        self.response = discord.ui.TextInput(label='自動回答', placeholder='このキーワードが含まれたら送信する文章', max_length=2000, required=True, style=discord.TextStyle.paragraph)
        self.add_item(self.keyword)
        self.add_item(self.response)

    async def on_submit(self, interaction):
        items = auto_answers(get_guild_config(interaction.guild.id))
        keyword = str(self.keyword).strip()
        response = str(self.response).strip()
        if len(items) >= 25:
            await interaction.response.send_message('❌ 自動回答は最大25個までです。', ephemeral=True); return
        if any(x['keyword'].casefold() == keyword.casefold() for x in items):
            await interaction.response.send_message('❌ そのキーワードはすでに登録されています。', ephemeral=True); return
        items.append({'keyword': keyword, 'response': response})
        set_guild_value(interaction.guild.id, 'ticket_auto_answers', items)
        await interaction.response.send_message(f'✅ **{keyword}** の自動回答を追加しました。', ephemeral=True)


class AutoAnswerEditModal(discord.ui.Modal):
    def __init__(self, index, current_keyword, current_response):
        super().__init__(title='自動回答を編集')
        self.index = index
    ...
    async def on_submit(self, interaction):
        ...
        for i, item in enumerate(items):
            if i != self.index and item['keyword'].casefold() == keyword.casefold():
                await interaction.response.send_message(
                    '❌ そのキーワードはすでに登録されています。',
                    ephemeral=True,
                )
...
class AutoAnswerEditView(discord.ui.View):
    def __init__(self, items):
        ...
        self.add_item(AutoAnswerEditSelect(items))


class AutoAnswerDeleteSelect(discord.ui.Select):
    def __init__(self, items):
        super().__init__(placeholder='削除する自動回答を選択', options=[discord.SelectOption(label=x['keyword'][:100], value=str(i), emoji='🗑️') for i,x in enumerate(items[:25])], min_values=1, max_values=1)
    async def callback(self, interaction):
        items = auto_answers(get_guild_config(interaction.guild.id))
        try: removed = items.pop(int(self.values[0]))
        except (ValueError, IndexError):
            await interaction.response.send_message('❌ 自動回答の取得に失敗しました。', ephemeral=True); return
        set_guild_value(interaction.guild.id, 'ticket_auto_answers', items)
        await interaction.response.send_message(f'🗑️ **{removed["keyword"]}** の自動回答を削除しました。', ephemeral=True)


class AutoAnswerDeleteView(discord.ui.View):
    def __init__(self, items):
        super().__init__(timeout=120); self.add_item(AutoAnswerDeleteSelect(items))
...
class TicketSettingsView(discord.ui.View):
    ...
    @discord.ui.button(label='自動回答追加', emoji='🤖', style=discord.ButtonStyle.success, row=4)
    async def auto_answer_add(self, i, b):
        await i.response.send_modal(AutoAnswerAddModal())
    @discord.ui.button(label='自動回答削除', emoji='🗑️', style=discord.ButtonStyle.danger, row=4)
    async def auto_answer_delete(self, i, b):
        items = auto_answers(get_guild_config(i.guild.id))
        if not items:
            await i.response.send_message('❌ 登録されている自動回答がありません。', ephemeral=True); return
        await i.response.send_message('🗑️ 削除する自動回答を選択してください。', view=AutoAnswerDeleteView(items), ephemeral=True)
    @discord.ui.button(label='自動回答編集', emoji='✏️', style=discord.ButtonStyle.primary, row=5)
    async def auto_answer_edit(self, i, b):
        items = auto_answers(get_guild_config(i.guild.id))
        if not items:
            await i.response.send_message('❌ 登録されている自動回答がありません。', ephemeral=True); return
        await i.response.send_message('✏️ 編集する自動回答を選択してください。', view=AutoAnswerEditView(items), ephemeral=True)
    @discord.ui.button(label='閉じるボタン', emoji='🔒', style=discord.ButtonStyle.secondary, row=6)
    async def close_label(self, i, b): await self.modal(i, 'ticket_close_label', '閉じるボタンを編集', 'ボタン文字', 80)
    @discord.ui.button(label='最初の自動返信', emoji='💬', style=discord.ButtonStyle.secondary, row=6)
...
class Ticket(commands.Cog):
    ...
    async def ticket_disable(self, interaction):
        for k in ['ticket_category_id','ticket_staff_role_ids','ticket_support_categories','ticket_title','ticket_message','ticket_button_label','ticket_close_label','ticket_embed_color','ticket_greeting','ticket_name','ticket_first_reply','ticket_auto_answers']:
...
class TicketMessageListener(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        staff_ids = c.get('ticket_staff_role_ids', [])
        is_staff_message = isinstance(message.author, discord.Member) and staff(message.author, staff_ids)

        # 自動回答はスタッフの発言には反応しない
        if not is_staff_message:
            content = message.content.strip()
            if content:
                answers = auto_answers(c)
                matched = []
                lowered = content.casefold()
                for item in answers:
                    if item['keyword'].casefold() in lowered:
                        matched.append(item['response'])
                if matched:
        ...
        # 従来の「最初のメッセージ」への自動返信
        if replied or message.author.id != creator:
            return
        ...
        if c['ticket_first_reply'].strip():
            try:
                await message.channel.send(replace_placeholders(c['ticket_first_reply'], message.author))
            except discord.HTTPException:
                return

        try:
            await message.channel.edit(topic=topic(creator, True))
        except discord.HTTPException:
            pass
